...
Use the cv.glmnet function from the glmnet library to run a 10-fold cross validation and select the best regularization parameter for the logistic regression with LASSO penalty. Set lambda = seq(1, 20) * 1e-5 in cv.glmnet() function to set pre-defined candidate values for the tuning parameter .
```{r}
set.seed(123)
...
What is the optimal value of in cross validation? 
A: best $\lambda$ from cross validation is 0.00019.

What are the non-zero coefficients in the LASSO regression for the optimal value of ? 
...
Tackle at least one more interesting question. Creative and thoughtful analysis will be rewarded! Some possibilities for further exploration are:
